...
export default function AdminServices() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowCreate(false);
      setForm(emptyForm);
      setSuccessMsg('Service catalog item created successfully!');
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Publish SaaS tools, cloud infrastructure, and recurring subscription plans for resellers.
          </p>
        </div>
        <button
          type="button"
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 space-y-3">
            ...
            <p className="text-xs text-slate-400 max-w-sm mx-auto">
              Create your first recurring SaaS service to populate the catalog.
            </p>
            <button
              onClick={() => {
                setShowCreate(true);
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50 text-slate-500 font-semibold uppercase tracking-wider">
                  <th className="px-4 py-3.5">Service Offering</th>
                    ...
                    <tr key={s.id} className="hover:bg-slate-50/70 transition-colors">
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-indigo-600 to-violet-600 text-white flex items-center justify-center font-bold text-sm shadow-xs shrink-0">
                          ...
                          <div>
                            ...
                            <div className="text-[11px] text-slate-400 font-mono">
                              /{s.slug} • {plan?.name || 'Standard Plan'}
                            </div>
                          </div>
                        </div>
                      </td>
          ...
          <div className="bg-white rounded-3xl max-w-xl w-full p-6 shadow-2xl border border-slate-200 space-y-5 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Configure catalog service details and wholesale tiers</p>
                </div>
              </div>
              <button
                type="button"
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                <div>
                  ...
                  <input
                    type="text"
                    required
                    placeholder="e.g. AWS VPS Enterprise Hosting"
                    value={form.name}
                    onChange={e => {
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="High-speed managed cloud instance with 99.9% uptime"
