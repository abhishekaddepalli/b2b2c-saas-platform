...
export default function ResellerServices() {
  ...
  const { data: servicesData, isLoading } = useQuery({
    ...
    queryFn: () => resellerApi.services({ search, per_page: 50 }).then(r => r.data),
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Search & Stats Bar */}
      <div className="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-2xs flex flex-col sm:flex-row gap-3 items-center justify-between">
        <div className="relative w-full sm:w-80">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
          <input
            type="text"
            placeholder="Search cloud services, compute, CRM…"
            value={search}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading ? (
          ...
          services.map(s => {
            ...
            const categoryName = typeof s.category === 'object' ? s.category?.name : (s.category || 'Cloud Service');
